...
class TableBoy extends Component{
    ...
    fetchPage(page_no,sort_on){
        ...
        this.state.sort_order=="DESC" ? this.setState({sort_order: "ASC"}) : this.setState({sort_order: "DESC"})
    ...
    renderTitle(titles){
        if(titles){
            return titles.map((title,index) =>(
                title.sortable?
                    <th key={index} style={{cursor: 'pointer'}} className={this.state.sort_order=="DESC" ? "responsive-table-sort sort-asc":"responsive-table-sort sort-desc"} onClick={()=>this.fetchPage(this.state.page_no,title.attribute)}>
                        {title.title}
                    </th>
                :
